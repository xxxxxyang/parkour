...
class Go2DistillLeapCfg( Go2LeapCfg ):
    ...
    class terrain( Go2LeapCfg.terrain ):
        if multi_process_:
            num_rows = 4
            num_cols = 1
        else:
        ...
        BarrierTrack_kwargs = merge_dict(Go2LeapCfg.terrain.BarrierTrack_kwargs, dict(
            leap= dict(
                length= [0.05, 0.8],
                depth= [0.5, 0.8],
    ...
    class sensor( Go2LeapCfg.sensor ):
    ...
    class commands( Go2LeapCfg.commands ):
        ...
        lin_cmd_cutoff = 0.2
        class ranges( Go2LeapCfg.commands.ranges ):
            # lin_vel_x = [0.6, 1.8]
            lin_vel_x = [-0.6, 2.0]
    ...
    class normalization( Go2LeapCfg.normalization ):
        class obs_scales( Go2LeapCfg.normalization.obs_scales ):
            forward_depth = 1.0

    class noise( Go2LeapCfg.noise ):
        add_noise = False
        class noise_scales( Go2LeapCfg.noise.noise_scales ):
            forward_depth = 0.0
    ...
    class sim( Go2LeapCfg.sim ):
        no_camera = False
    
logs_root = osp.join(osp.dirname(osp.dirname(osp.dirname(osp.dirname(osp.abspath(__file__))))), "logs")
class Go2DistillLeapCfgPPO( Go2LeapCfgPPO ):
    class algorithm( Go2LeapCfgPPO.algorithm ):
        entropy_coef = 0.0
        using_ppo = False
